...
root = etree.parse('../steps/3-results/results.xml').getroot()
...
plt.savefig('../steps/3-results/figures/venn_clustered.pdf')
plt.clf()
...
for patch in v.patches:
    patch.set_edgecolor('black')
plt.savefig('../steps/3-results/figures/venn_raw.pdf')
...
def write_freqs(l, filename):
    with open('../steps/3-results/frequencies/' + filename, 'w') as f:
